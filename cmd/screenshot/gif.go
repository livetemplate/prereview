...
// capture grabs the current viewport, downscales it to gifWidth, quantizes it,
// and appends it as a frame held for holdCs centiseconds (1cs = 10ms).
...
func runGif(allocCtx context.Context, url, name, outDir string) {
	switch name {
	case "hero":
		gifHero(allocCtx, url, outDir)
	case "image":
...
// gifHero — the review→fix loop on a code diff: open the diff, select a line,
// type a comment, save it, hand off to Claude (toast confirms the CSV write).
func gifHero(allocCtx context.Context, url, outDir string) {
	withCtx(allocCtx, "gif:hero", func(ctx context.Context) error {
		rec := &gifRec{}
		// Open payment.go's diff.
		if err := chromedp.Run(ctx, base(url, "payment.go", 1280, 800)...); err != nil {
			return err
		}
		if err := rec.capture(ctx, 130); err != nil { // the diff, before any comment
			return err
		}
		// Select line 18 via the deep-link hash → composer opens.
		if err := chromedp.Run(ctx,
			chromedp.Navigate(navURL(url, "payment.go:L18")),
		...
		_ = rec.capture(ctx, 80) // empty composer

		// Type the comment in two chunks for a "typing" feel.
		_ = chromedp.Run(ctx, chromedp.SendKeys(`.composer textarea`, "Surface the gateway's real error ", chromedp.ByQuery), chromedp.Sleep(150*time.Millisecond))
		_ = rec.capture(ctx, 45)
		_ = chromedp.Run(ctx, chromedp.SendKeys(`.composer textarea`, "here, not a generic string.", chromedp.ByQuery), chromedp.Sleep(150*time.Millisecond))
		_ = rec.capture(ctx, 140) // full comment
		...
		_ = rec.capture(ctx, 120)

		// Hand off → Claude: writes the marker and shows the "Saved" toast.
		_ = chromedp.Run(ctx, chromedp.Click(`header.bar button[name='handOff']`, chromedp.ByQuery), chromedp.Sleep(800*time.Millisecond))
		_ = rec.capture(ctx, 200) // hold on the toast

		return rec.encode(filepath.Join(outDir, "hero.gif"))
	})
}
