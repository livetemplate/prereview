...
// mode keeps region comments diff-free so flushSnapshot's relocateAll is a no-op.
...
// TestFlushHandoff_EmitsActionableSnapshot pins that a snapshot flush emits
// exactly one snapshot event whose snapshot is the actionable comments only —
...
func TestFlushHandoff_EmitsActionableSnapshot(t *testing.T) {
	...
	if len(evs) != 1 || evs[0].Event != "snapshot" {
		t.Fatalf("want 1 snapshot event, got %+v", evs)
	}
	if evs[0].Seq != 0 {
...
func TestFlushHandoff_SeqIncrements(t *testing.T) {
	...
	if err := c.flushSnapshot(&st); err != nil {
		t.Fatalf("first flushSnapshot: %v", err)
	}
	// User resolves "a" in the UI, then hands off again.
	st.Comments[0].Resolved = true
	if err := c.flushSnapshot(&st); err != nil {
		t.Fatalf("second flushSnapshot: %v", err)
	}

	evs := decodeEvents(t, buf.Bytes())
	if len(evs) != 2 {
		t.Fatalf("want 2 snapshot events, got %d", len(evs))
	}
...
// sequence: End session flushes a final snapshot (so comments left
// since the last Hand off still reach the consumer — the footgun fix), then
// emits end, sets the SessionEnded banner flag, and requests a
// graceful shutdown.
func TestEndSession_FlushesThenTerminatesAndShutsDown(t *testing.T) {
	...
	evs := decodeEvents(t, buf.Bytes())
	if len(evs) != 2 {
		t.Fatalf("want [handoff, end], got %+v", evs)
	}
	if h := evs[0].CommentList(); evs[0].Event != "snapshot" || len(h) != 1 || h[0].ID != "late" {
		t.Errorf("End session should flush the pending comment first, got %+v", evs[0])
	}
	if evs[1].Event != "end" {
		t.Errorf("terminator must be end, got %q", evs[1].Event)
	}
...
// emitter is safe — flushSnapshot persists the CSV and emits nothing rather than
// panicking.
func TestFlushHandoff_NilEmitterSafe(t *testing.T) {
	...
	if err := c.flushSnapshot(&st); err != nil {
		t.Fatalf("flushSnapshot with nil emitter: %v", err)
	}
}
