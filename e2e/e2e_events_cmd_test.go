...
// comments --json` enumerates it, `prereview watch --since` delivers the
// snapshot event, and `prereview done` validates ids (a real id marks; a
...
func TestE2E_AgentSubcommands(t *testing.T) {
	...
	// `prereview watch --since 0` — cursor past ready@0, so the reader BLOCKS
	...
	out, _, exit := runCLI(t, 10*time.Second, p.binary, "watch", "--out", p.repo, "--since", "0")
	if exit != 0 {
	...
	// `prereview done <id>` — a real id marks worked-on (exit 0).
	if _, errb, exit := runCLI(t, 5*time.Second, p.binary, "done", "--out", p.repo, id); exit != 0 {
		t.Fatalf("processed <real-id> exit=%d\nstderr: %s", exit, errb)
	}

	// `prereview done <bogus>` — the #1 regression: an unknown id fails
	// non-zero against the real store, instead of silently recording garbage.
	if _, errb, exit := runCLI(t, 5*time.Second, p.binary, "done", "--out", p.repo, "totally-bogus-xyz"); exit == 0 {
